...
float   topWallMass = 1.0f*(RHO_L)*0.6*OVER_PRESSURE*0.1408*0.0352;//0.1f*0.34315f;// 4.0f* (2.0f+1.0f)/2.0f * 0.34315; //2.0f * 5.0f / 2.0f * (0.49078f/2.0f*64.0f/48.0f); //kg; equal to some value times the total weight of the particle bed; divided by two b/c I halved bed height; multiplied by 5/2 to make up for heavy particles with 4x density;
float   topWallAcl = 0.0f;
float shearRate=5.0f;
...
ParticleSystem::_initialize(int numParticles)
{
	...
	memset(m_hheight,0,m_numParticles*sizeof(float)*4);
	...
	memset(m_hstresssum, 0, m_numParticles*9*sizeof(float));
	...
	allocateArray((void **)&m_dheight, sizeof(float)* m_numParticles*4);
	allocateArray((void **)&m_dyforce, sizeof(float)* m_numParticles);
	allocateArray((void **)&m_dstresssum, m_numParticles*9*sizeof(float));
...
//		printf("Total Normal Force d %f\n",*m_dTotalNormalForce); //print total normal force,to check what it is
	...
	if (stepCount>0.8*ASD &&  stepCount<1.8*ASD )
	{
...
//		topWallVel += 0.5f*(topWallAclLast+topWallAcl)*deltaTime;
		...
		float* pos;
		pos = getArray(POSITION);
		float nl[num_of_bins]={0};
		float ns[num_of_bins]={0};
...
ParticleSystem::write2Bin()
{
	...
	cudaMemcpy(m_hheight, m_dheight, sizeof(float)*m_numParticles*4, cudaMemcpyDeviceToHost);
...
//			std::cout<<std::endl;
	...
	for (int i = 0; i < m_ntotal; i++)
	{
		 float buffer[12] = {pos[i*4+3],(float)i,vel[i*4+3],pos[i*4+0],pos[i*4+1],pos[i*4+2],vel[i*4+0],vel[i*4+1],vel[i*4+2],m_hheight[i*4],m_hheight[i*4+2],m_hyforce[i]}; // radius, index, color, x, y, z, vx, vy, vz, ovx, ovy, ovz
		//float buffer[12] = {pos[i*4+3],(float)i,vel[i*4+3],pos[i*4+0],pos[i*4+1],pos[i*4+2],vel[i*4+2],m_hheight[i*4],m_hheight[i*4+1],m_hheight[i*4+2],m_hheight[i*4+3],m_hyforce[i]}; // radius, index, color, x, y, z, vx, vy, vz, ovx, ovy, ovz
...
void
ParticleSystem::write2Binstress()
{
	float*tot = new float[1];
...
//	printf("I'm actually here!!!!");

	pFile = fopen("stress.bin", "a+b");

//	printf("I'm here!!!!");

	copyArrayFromDevice(m_hstresssum, m_dstresssum, &cuda_vbo_resource, sizeof(float)*9*m_numParticles);

	/*printf("pos: (%.8f, %.8f, %.8f, %.8f)\n", pos[0 * 4 + 0], pos[0 * 4 + 1], pos[0 * 4 + 2], pos[0 * 4 + 3]);
	printf("vel: (%.8f, %.8f, %.8f, %.8f)\n", vel[0 * 4 + 0], vel[0 * 4 + 1], vel[0 * 4 + 2], vel[0 * 4 + 3]);
	printf("ang vel: (%.8f, %.8f, %.8f, %.8f)\n", m_hOmegVel[0 * 4 + 0], m_hOmegVel[0 * 4 + 1], m_hOmegVel[0 * 4 + 2], m_hOmegVel[0 * 4 + 3]);*/
...
//	printf("I'm here!!!!");
	...
	for (int i = 0; i < m_ntotal; i++)
	{
		float buffer[13] = {vel[i*4+3],pos[i*4+0],pos[i*4+1],pos[i*4+2],(float)m_hstresssum[i*9+0],(float)m_hstresssum[i*9+1],(float)m_hstresssum[i*9+2],(float)m_hstresssum[i*9+3],(float)m_hstresssum[i*9+4],(float)m_hstresssum[i*9+5],(float)m_hstresssum[i*9+6],(float)m_hstresssum[i*9+7],(float)m_hstresssum[i*9+8]};
		fwrite(buffer, sizeof(float), 13, pFile);
		//fprintf(pFile, "%f %f %f %f %f %f %f %f %f \n", pos[i*4+3],(double)i,vel[i*4+3],pos[i*4+0],pos[i*4+1],pos[i*4+2],vel[i*4+0],vel[i*4+1],vel[i*4+2]);
	}
	fclose(pFile);
	return;
}
...
//	Creates large particles in grid, at random locations, in order to make prescribed large/small particle ratio
		...
		do
		{
			...
			// 		i++;
			// 	}
								if (m_hVel[int(i*10*256*12+1000*h*2)*4+3]==1.0f){
									m_hVel[int(i*10*256*12+1000*h*2)*4+3]=2.0f;
									i++;
								}
		}
		while (i < int(4));
...
//				m_hMass[i] = 4.0f / 3.0f * CUDART_PI_F * powf(m_hPos[i*4+3],3) * 2500.0f; //size bidisperse
				m_hPos[i*4+3] =  largeParticleRadius*1.5f; //Density bi
