			  ...
			  float   *stresssum)
{
   ...
   stresssum[originalIndex*9] = 0;
   stresssum[originalIndex*9+1] = 0;
   stresssum[originalIndex*9+2] = 0;

   stresssum[originalIndex*9+3] = 0;
   stresssum[originalIndex*9+4] = 0;
   stresssum[originalIndex*9+5] = 0;

   stresssum[originalIndex*9+6] = 0;
   stresssum[originalIndex*9+7] = 0;
   stresssum[originalIndex*9+8] = 0;
   ...
   for (uint i=0; i < numberOfNeighbors; i++)
   {
	   ...
	   if(nebId < numParticles)
	   {
           ...
           stresssum[originalIndex*9] = stresssum[originalIndex*9]     + forces.force.x ;
           stresssum[originalIndex*9+1] = stresssum[originalIndex*9+1] + forces.force.x ;
           stresssum[originalIndex*9+2] = stresssum[originalIndex*9+2] + forces.force.x ;

           stresssum[originalIndex*9+3] = stresssum[originalIndex*9+3] + forces.force.y ;
           stresssum[originalIndex*9+4] = stresssum[originalIndex*9+4] + forces.force.y ;
           stresssum[originalIndex*9+5] = stresssum[originalIndex*9+5] + forces.force.y ;

           stresssum[originalIndex*9+6] = stresssum[originalIndex*9+6] + forces.force.z ;
           stresssum[originalIndex*9+7] = stresssum[originalIndex*9+7] + forces.force.z ;
           stresssum[originalIndex*9+8] = stresssum[originalIndex*9+8] + forces.force.z ;
	...
	if (stepCount > 1.0*ASD)
	{
		float kNudge = 0.01f+0.02f*(height-unsortedPos[originalIndex].y)/height; //proportional "nudge coefficient" for enforcing velocity profile
		//profile linear
	...
	if (stepCount > 1.0f*ASD && unsortedVel[originalIndex].w>1.5f)
	{
	// force = make_float3(0.0f);
	// torque = make_float3(0.0f);
	unsortedVel[originalIndex].x=0;
	unsortedVel[originalIndex].y=0;
	unsortedVel[originalIndex].z=unsortedPos[originalIndex].y*shearRate+VSLIP*0.04f;
